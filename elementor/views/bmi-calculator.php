<?php
/**
 * @author  RadiusTheme
 * @since   1.0
 * @version 1.0
 */

$uniqid = (int) wp_rand();

$metric_checked = $imperial_checked = $metric_style = $imperial_style = '';
if ( $data['unit_default'] == 'imperial' ) {
    $imperial_checked = ' checked';
    $metric_style = 'display:none;';
} else {
    $metric_checked = ' checked';
    $imperial_style = 'display:none;';
}

$metric_radio_html = '<input class="form-check-input" id="rt-bmi-metric-' . $uniqid . '" type="radio" name="rt-bmi-unit" value="metric"' . $metric_checked . '><label for="rt-bmi-metric-' . $uniqid . '">' . __( 'Metric Units', 'gymat-core' ) . '</label>';
$imperial_radio_html = '<input class="form-check-input" id="rt-bmi-imperial-' . $uniqid . '" type="radio" name="rt-bmi-unit" value="imperial"' . $imperial_checked . '><label for="rt-bmi-imperial-' . $uniqid . '">' . __( 'Imperial Units', 'gymat-core' ) . '</label>';

if ( $data['unit_default'] == 'imperial' ) {
    $radio_html = $imperial_radio_html . $metric_radio_html;
} else {
    $radio_html = $metric_radio_html . $imperial_radio_html;
}
$bmi_chart = array(
    array( __( 'Below 18.5', 'gymat-core' ),   __( 'Underweight', 'gymat-core' ) ),
    array( __( '18.5 - 24.9', 'gymat-core' ),  __( 'Normal', 'gymat-core' ) ),
    array( __( '25 - 29.9', 'gymat-core' ),  __( 'Overweight', 'gymat-core' ) ),
    array( __( '30 and Above', 'gymat-core' ), __( 'Obese', 'gymat-core' ) ),
);
$bmi_chart_encoded = json_encode( $bmi_chart );
?>
<div class="rt-bmi-calculator <?php echo esc_attr( $data['style'] ); ?>">
    <div class="row">
        <div class="col-lg-6">
            <div class="rt-left">
                <h2 class="rt-title"><?php echo esc_html( $data['title'] ); ?></h2>
                <div class="rt-subtitle"><?php echo wp_kses_post($data['content']); ?></div>
                <form class="rt-bmi-form">
                    <div class="rt-bmi-radio"<?php echo ( $data['unit_display'] != 'yes' ) ? ' style="display:none;"':'';?>>
                        <?php echo wp_kses( $radio_html, array( 'input' => array( 'class' => true, 'id' => true, 'type' => true, 'name' => true, 'value' => true, 'checked' => true ), 'label' => array( 'for' => true ) ) ); ?>
                    </div>
                    <div class="rt-bmi-fields">
                        <input type="text" class="rt-bmi-fields-metric form-control" name="rt-bmi-weight" placeholder="<?php esc_attr_e( 'Weight / kg', 'gymat-core' ); ?>" style="<?php echo esc_attr( $metric_style ); ?>">
                        <input type="text" class="rt-bmi-fields-metric form-control" name="rt-bmi-height" placeholder="<?php esc_attr_e( 'Height / cm', 'gymat-core' ); ?>" style="<?php echo esc_attr( $metric_style ); ?>">
                        <input type="text" class="rt-bmi-fields-imperial form-control" name="rt-bmi-pound" placeholder="<?php esc_attr_e( 'Weight / lbs', 'gymat-core' ); ?>" style="<?php echo esc_attr( $imperial_style ); ?>">
                        <input type="text" class="rt-bmi-fields-imperial form-control" name="rt-bmi-feet" placeholder="<?php esc_attr_e( 'Height / feet', 'gymat-core' ); ?>" style="<?php echo esc_attr( $imperial_style ); ?>">
                        <input type="text" class="rt-bmi-fields-imperial form-control" name="rt-bmi-inch" placeholder="<?php esc_attr_e( 'Height / inch', 'gymat-core' ); ?>" style="<?php echo esc_attr( $imperial_style ); ?>">
                    </div>
                    <input type="submit" class= "rt-bmi-submit" value="<?php echo esc_html( $data['btn_text'] ); ?>">
                    <div class="rt-bmi-result" style="display:none;" data-chart="<?php echo esc_attr( $bmi_chart_encoded ); ?>"><?php esc_html_e( 'Your BMI is:', 'gymat-core' );?> <span class="rt-bmi-val"></span><?php esc_html_e( ', and weight status is:', 'gymat-core' );?> <span class="rt-bmi-status"></span></div>
                    <div class="rt-bmi-error" data-emptymsg="<?php esc_attr_e( 'Error: One or more fields are empty', 'gymat-core' );?>" data-numbermsg="<?php esc_attr_e( 'Error: All field values must be a number', 'gymat-core' );?>"></div>
                </form>
            </div>
        </div>
        <div class="col-lg-6">
            <div class="rt-right">
                <table class="bmi-chart">
                    <tr>
                        <th><?php esc_html_e( 'BMI', 'gymat-core' );?></th>
                        <th><?php esc_html_e( 'Weight Status', 'gymat-core' );?></th>
                    </tr>
                    <?php foreach ( $bmi_chart as $chart ): ?>
                        <tr>
                            <td><?php echo esc_html( $chart[0] ); ?></td>
                            <td><?php echo esc_html( $chart[1] ); ?></td>
                        </tr>
                    <?php endforeach; ?>
                </table>
            </div>
            <div class="bmi-chart-title"> 
				<h3><span><?php esc_html_e('BMR','gymat-core'); ?></span> <?php esc_html_e('Metabolic Rate','gymat-core'); ?> / <span><?php esc_html_e('BMI','gymat-core'); ?></span> <?php esc_html_e('Body Mass Index','gymat-core'); ?></h3>
			</div>
        </div>
    </div>
</div>